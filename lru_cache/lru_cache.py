class LRUCache:
    ...
    def set(self, key, value):
        # check if the key is in the cache we need to overwrite the old value
        if key in self.storage
            Node.value = (key, value)
            self.ordering.move_to_end(node)

        # Must first evict least-recently used element
        if self.size == self.limit:
            oldest_key = self.ordering.head.value[0] # head is the oldest element
